const reviewsData = [
  {
    id: "gameA",
    title: "『ゲームA』レビュー",
    date: "2026年6月",
    image: "game_review_site_images/87306.jpg",
    category: "RPG",
    content: "ここにゲームAの詳しい評価や、グラフィック、システムについての詳細な文章"
  },

  {
    id: "monsterhunterWilds",
    title: "モンスターハンターワイルズ",
    date: "2026年6月26日",
    category: "ハンティングアクション",
    content: "　2027年に大型追加コンテンツの発売が決定したモンスターハンターシリーズ最新作。今作はsteamでの評価が賛否両論であった。それはなぜなのだろうか。<br>　一つ目がモンスター数が残念だったことである。"
  },

  {
    id: "xenoblade-definitive-edition",
    title: "Xenoblade Definitive Edition",
    date: "2026年6月26日",
    category: "RPG",
    content: "　「Xenoblade Definitive Edition」は、2010年にWii向けのゲームタイトルとして発売された「Xenoblade」のリメイク版として、2020年にNintendo Switch向けタイトルとして発売された。\nリメイク前と比べてUIやグラフィックが改善されたことで、より遊びやすく、Xenobladeの世界をよりきれいな映像で楽しめるようになった。"
  }
];

/*記事を書くときは一番上の書き方を参考にしてください！*/